...
```{mermaid}
graph LR
  A[My local computer 💻] --> B(my_project) --> C(downloaded_data) --> D(landsat_data)
  D --> E(B01)
  D --> F(B02)
  E --> G(example_raster.tif)
  F --> H(another_example_raster.tif)
```
...
```{mermaid}
graph LR
  A[MaloneLab Server ☁️] --> B(Research) --> C("my_project (can be renamed to differentiate from repo name)") --> D(downloaded_data) --> E(landsat_data)
  E --> F(B01)
  E --> G(B02)
  F --> H(example_raster.tif)
  G --> I(another_example_raster.tif)
```
